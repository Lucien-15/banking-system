///For defining tjhe structured data of the Accoount Number
use rand::prelude::*;
use std::fmt::Display;
use std::str::FromStr;
/// the account number uses random number generator
#[derive(Debug)]
pub struct AccountNumber {
    ///Ramdomly generated number/ Each digitis within 0..9
...
///For using the FromStr trait to transform in account numbers from strings
    ...
    fn from_str(s: &str) -> Result<Self, std::io::ErrorKind> {
        if verify(s) {
            let mut payload: Vec<u8> = s
                ...
                .collect();
            let check_digit = payload.pop().expect("Cannot pop. Got an empoty vector");
            Ok(Self {
                payload,
                check_digit,
            })
...
///Helper fuctio to verify if string is valid AccountNumber
...
pub fn verify(account_number: &str) -> bool {
    ...
    let check_digit = payload.pop().expect("This shouldnt be an empty iterator");
...
///Tests for Luhn number algorithm
